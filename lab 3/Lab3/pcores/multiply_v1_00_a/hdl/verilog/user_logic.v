...
//----------------------------------------------------------------------------
  ...
  always @ (posedge Bus2IP_Clk)
		begin
			if (Bus2IP_Reset==1)
				begin
					slv_reg2<=0; //if reset set output and temp reg to 0
					tmp_reg<=0;
				end
			else
				begin
					tmp_reg<= slv_reg0*slv_reg1; //multiply arguments store in internal temp reg
					slv_reg2<= tmp_reg;// set output to internal reg
				end //this separates the i/o from the multiply, allowing it to be completed in one clock period
		end
